...
const follow = () => {
  ...
  // [URL]
  const followUrl = `https://teachapi.herokuapp.com/users/${number}/follow`;

  // 登録APIを呼び出す
  follow_common(followUrl).then(result => {
    console.log(result)
  });
}
...
const myFollow = () => {
  ...
  // [URL]
  const followerUrl = `https://teachapi.herokuapp.com/users/${number}/followings`;

  // 登録APIを呼び出す
  myFollow_common(followerUrl).then(result => {
    console.log(result)
  });
}
//投稿削除できない問題
